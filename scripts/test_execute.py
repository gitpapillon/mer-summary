...
def executor(tmp_project, phase_dir):
    ...
    git 내부 동작(브랜치/커밋) 검증을 위해 _use_git=True 로 둔다.
    git-optional(off) 동작은 TestGitOptional 에서 별도로 검증한다.
    """
...
class TestLoadGuardrails:
    ...
    def test_empty_project(self, tmp_path):
        # CLAUDE.md / docs 가 없는 프로젝트 → 가드레일 빈 문자열
...
class TestMainCli:
    ...
    def test_invalid_phase_dir_exits(self, tmp_path):
        argv = ["execute.py", "nonexistent", "--project", str(tmp_path)]
    ...
    def test_missing_index_exits(self, tmp_project):
        ...
        argv = ["execute.py", "empty", "--project", str(tmp_project)]
        with patch("sys.argv", argv):
            with pytest.raises(SystemExit) as exc_info:
                ex.main()
            assert exc_info.value.code == 1
...
# ---------------------------------------------------------------------------
# git-optional 동작 (--git / --push 없으면 git 미사용)
# ---------------------------------------------------------------------------

class TestGitOptional:
    def _spy_git(self, executor):
        calls = []
        executor._run_git = lambda *a: calls.append(a) or MagicMock(
            returncode=0, stdout="", stderr=""
        )
        return calls

    def test_checkout_branch_noop_when_git_off(self, executor):
        executor._use_git = False
        calls = self._spy_git(executor)
        executor._checkout_branch()  # 예외/exit 없이 즉시 반환
        assert calls == []

    def test_commit_step_noop_when_git_off(self, executor):
        executor._use_git = False
        calls = self._spy_git(executor)
        executor._commit_step(2, "ui")
        assert calls == []

    def test_checkout_branch_runs_when_git_on(self, executor):
        executor._use_git = True
        calls = self._spy_git(executor)
        executor._run_git = lambda *a: calls.append(a) or MagicMock(
            returncode=0, stdout="feat-mvp\n", stderr=""
        )
        executor._checkout_branch()  # git on → rev-parse 호출됨
        assert calls and calls[0][0] == "rev-parse"


# ---------------------------------------------------------------------------
# 재사용성 (--project / git 플래그 → 인스턴스 상태)
# ---------------------------------------------------------------------------

class TestProjectRoot:
    def test_project_root_overrides_default(self, tmp_project, phase_dir):
        inst = ex.StepExecutor("0-mvp", project_root=str(tmp_project))
        assert inst._project_root == tmp_project.resolve()
        assert inst._phases_dir == tmp_project.resolve() / "phases"
        assert inst._root == str(tmp_project.resolve())

    def test_git_off_by_default(self, tmp_project, phase_dir):
        inst = ex.StepExecutor("0-mvp", project_root=str(tmp_project))
        assert inst._use_git is False

    def test_use_git_flag(self, tmp_project, phase_dir):
        inst = ex.StepExecutor("0-mvp", project_root=str(tmp_project), use_git=True)
        assert inst._use_git is True

    def test_auto_push_implies_use_git(self, tmp_project, phase_dir):
        inst = ex.StepExecutor("0-mvp", project_root=str(tmp_project), auto_push=True)
        assert inst._use_git is True
        assert inst._auto_push is True
