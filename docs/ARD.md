...
### ARD-005: 입력은 CLI 인자 1개 URL — 네이버 메인이면 N개 자동 처리
**결정**: `mer-summary <URL>` 형태. URL 1개만 받음.
- 네이버 블로그 메인 패턴 감지 시 당일 글 모두를 자동 순회 처리. 허용 패턴:
  - `https://blog.naver.com/{blogId}` (PC 도메인)
  - `https://m.blog.naver.com/{blogId}` (모바일 도메인)
  - 위 + `?categoryNo={N}&...` 쿼리 → 해당 카테고리만 조회.
- 그 외 URL: 그 1개만 처리 (`fetch_article` 라우터).
**이유**: 사용 시나리오가 "이 블로그/카테고리 보내줘". RSS/배치는 외부 셸 루프나 cron으로 해결. 카테고리 분리는 메르처럼 한 블로그에서 주제별로 가져오기 위함.
**트레이드오프**: 일괄 처리는 외부에서 `for url in $(...); do mer-summary "$url"; done`. 다중 카테고리는 URL 여러 번 실행.
