...
`/docs/` 하위 문서(PRD, ARCHITECTURE, ARD, UI_GUIDE 등)를 읽고 프로젝트의 기획·아키텍처·설계 의도를 파악한다. 필요시 Explore 에이전트를 병렬로 사용한다.
...
```bash
python3 scripts/execute.py {task-name}                          # 현재 디렉토리 기준 순차 실행
python3 scripts/execute.py {task-name} --project ../app          # 다른 프로젝트 대상
python3 scripts/execute.py {task-name} --project ../app --git    # +브랜치/커밋
python3 scripts/execute.py {task-name} --project ../app --push   # +origin push
```

execute.py가 자동으로 처리하는 것:

- `--git`/`--push` 시: `feat-{task-name}` 브랜치 생성/checkout (git 미사용 시 생략)
